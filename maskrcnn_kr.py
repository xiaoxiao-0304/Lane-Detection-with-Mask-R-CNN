...
def train(model,args_dataset,epochs,lr):#,config):
    """Train the model."""
    # Training dataset.
    dataset_train = RiderDataset()
    dataset_train.load_rider(args_dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = RiderDataset()
    dataset_val.load_rider(args_dataset, "val")
    dataset_val.prepare()
...
def predict_and_show(model, image_path=None): 
    #import visualize_cv2 

    args_image=image_path

    # Validation dataset
    dataset = RiderDataset() 
    dataset.load_rider(None, "val") 
    ...
    # Read image
    image = skimage.io.imread(args_image)
    # Detect objects
    r = model.detect([image], verbose=1)[0]
    # Color splash
    splash = color_splash(image, r['masks'])
    ...
    filter_classs_names = ['roadmark']
    save_image(image, file_name, r['rois'], r['masks'],
...
class MaskRCNN(object):
    ...
    def __init__(self, mode, class_names):
        """
        ...
        ROOT_DIR = os.path.abspath(".")
        DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR, "logs")
        self.model_dir=DEFAULT_LOGS_DIR
        ...
        self.weights_path,self.is_found = self.find_last()
        weights_path=self.weights_path
        self.load_weights(self.weights_path, by_name=True)
...
class InferenceConfig(RiderConfig):
...
class RiderDataset(Dataset):
    def load_rider(self, dataset_dir, subset):
        """Load a subset of the Rider dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes.
        self.add_class("rider", 1, "roadmark")
        if not dataset_dir:
            return
        ...
        annotations = json.load(open(os.path.join(dataset_dir, "via_region_data_rider.json")))
        ...
        for a in annotations:
            ...
            class_ids = [int(n['road']) for n in objects]
...
while True:
    val=input('T for training,\
               P for predicting image,\
               E for exit:')
    if val=="T":
        #---train---
        model = MaskRCNN(
